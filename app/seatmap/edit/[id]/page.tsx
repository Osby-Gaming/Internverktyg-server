import { getRoom } from "@/lib/appwrite_server";
...
export default async function Page({
    ...
    if (id !== "new") {
        ...
        const cellsInDatabase = roomData.cells.sort((a: any, b: any) => a.index - b.index);
        console.log(cellsInDatabase);

        const cells: Cell[] = [];

        let lastInsertIndex = 0;

        for (let i = 0; i < cellsInDatabase.length; i++) {
            const cellInDb = cellsInDatabase[i];

            if (cellInDb.index > lastInsertIndex) {
                let startIndex = lastInsertIndex + 1;

                for (let j = startIndex; j < cellInDb.index; j++) {
                    cells.push(null);
                }
            }

            const cellToInsert: Cell = {
                name: cellInDb.name,
                type: cellInDb.type,
                styleOverride: JSON.parse(cellInDb.styleOverride) || {}
            }

            if (JSON.stringify(cellToInsert.styleOverride) === "{}") {
                delete cellToInsert.styleOverride;
            }

            cells.push(cellToInsert);
            lastInsertIndex = cellInDb.index;
        }
        ...
        mapLayout = {
            ...
            cells,
            globalOverride: {
                cellStyleOverride: roomData.cell_style_override,
                backgroundColor: roomData.background_color,
                zoomLevel: roomData.zoom_level
            }
        };
    }

    return (
        <Map mapLayout={mapLayout}></Map>
    );
}